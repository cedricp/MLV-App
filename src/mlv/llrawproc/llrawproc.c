...
#pragma omp parallel for schedule(static) default(none) shared(image_data, pixel_count) reduction(min : min_level) reduction(max : max_level)
...
#pragma omp parallel for schedule(static) default(none) shared(image_data, pixel_count, raw_info, scale_ratio)
    for(uint32_t i = 0; i < pixel_count; ++i)
    {
...
void applyLLRawProcObject(mlvObject_t * video, uint16_t * raw_image_buff, size_t raw_image_size)
{
    ...
    /* fix bad pixels */
    if (video->llrawproc->bad_pixels && video->llrawproc->bpm_status < 3 && (!video->llrawproc->dual_iso || (video->llrawproc->dual_iso && video->llrawproc->bad_pixels == 3))) //Only if map mode selected for Dual ISO. Otherwise the fix is done inside the dual ISO function
    {
    ...
    if(video->llrawproc->diso_validity && video->llrawproc->dual_iso)
    {
        ...
        if (video->llrawproc->dual_iso == 1) // Full 20bit processing mode
        {
            diso_get_full20bit(raw_info,
                               ...
                               video->llrawproc->bad_pixels,
                               video->llrawproc->bps_method,
                               video->llrawproc->diso_dh_threshold);
...
{
    return video->llrawproc->diso_dh_threshold;
}

void llrpSetDualDarkHighlightThresholdValue(mlvObject_t * video, int value)
{
    video->llrawproc->diso_dh_threshold = value;
}
